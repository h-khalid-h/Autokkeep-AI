...
export async function GET(request: NextRequest) {
  try {
    ...
    const [entitiesResult, txResult] = await Promise.all([
      admin.from('entities').select('id, org_id').in('org_id', orgIds),
      // Get latest transaction per entity for activity tracking
      admin
        .from('transactions')
    ...
    const txData = (txResult.data || []) as TxRow[];
    ...
    // Count transactions per org & find last activity
    ...
    for (const org of orgList) {
      ...
      if (orgEntityIds.length === 0) continue;
      const entityIdSet = new Set(orgEntityIds);
      for (const tx of txData) {
        if (entityIdSet.has(tx.entity_id)) {
          txCountByOrg[org.id] = (txCountByOrg[org.id] || 0) + 1;
          if (!lastActivityByOrg[org.id] || tx.created_at > lastActivityByOrg[org.id]) {
            lastActivityByOrg[org.id] = tx.created_at;
          }
        }
      }
    }
