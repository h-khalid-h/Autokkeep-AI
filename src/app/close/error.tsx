...
export default function CloseError({
  ...
  useEffect(() => {
    console.error('[MonthEndClose] Error:', error);
    import('@/lib/sentry').then(({ captureException }) => {
      captureException(error, {
        tags: { boundary: 'close-error' },
    ...
    >
      <div className="card-elevated" style={{ textAlign: 'center', maxWidth: '460px', padding: '48px 32px' }}>
        ...
        <h2 className="text-h3" style={{ marginBottom: '8px' }}>
          Month-End Close encountered an error
        </h2>
        <p
          className="text-body"
          style={{ marginBottom: '24px', color: 'var(--text-secondary)' }}
        >
          Something went wrong while loading the month-end close workflow. Your data is
          safe — please retry or return to the dashboard.
        </p>
